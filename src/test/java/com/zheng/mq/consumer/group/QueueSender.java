package com.zheng.mq.consumer.group;
...
/**
 * 消息分组，相同分组ID的消息将会发布到同一个consumer进行消费
 * @Author zhenglian
...
public class QueueSender {
    ...
    public void sendTextMessage() throws JMSException {
        TextMessage msg;
        for (int i = 0; i < 3; i++) {
            msg = session.createTextMessage("messageAA----" + (i + 1));
            // 设置消息所属组
            msg.setStringProperty("JMSXGroupID", "groupA");
            producer.send(msg);

            msg = session.createTextMessage("messageBB----" + (i + 1));
            // 设置消息所属组
            msg.setStringProperty("JMSXGroupID", "groupB");
            producer.send(msg);

            msg = session.createTextMessage("messageCC----" + (i + 1));
            // 设置消息所属组
            msg.setStringProperty("JMSXGroupID", "groupC");
            producer.send(msg);
        }
    }
